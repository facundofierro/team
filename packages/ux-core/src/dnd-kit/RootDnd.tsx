...
export function RootDnd({ children }: { children: React.ReactNode }) {
  ...
  const customCollisionDetector = (args: {
    ...
    if (outsideRef?.current !== nextIsOutside) {
      ;(outsideRef as any).current = nextIsOutside
    ...
    if (pointerInsideOrigin || activeItem?.type === 'template') {
      ...
      return rectIntersection({
        ...args,
        droppableContainers: droppableContainersToUse,
      })
    }

    droppableContainersToUse = droppableContainers.filter(
      (c) =>
        c.data.current?.type === 'container' &&
        isValidDropTarget(c.id.toString())
    )
  ...
  return (
    <DndContext
      sensors={sensors}
      modifiers={[
        ...(activeItem?.type === 'template' ? [restrictToParentElement] : []),
      ]}
