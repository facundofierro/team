...
export function DragStateProvider({ children }: { children: React.ReactNode }) {
  ...
  const [dropZoneStatuses, setDropZoneStatuses] = useState<
    Map<string, DropZoneStatus>
  >(new Map())
  const [isOutsideOfContainer, setIsOutsideOfContainer] = useState(false)

  const outsideRef = useRef<boolean | null>(null)
  ...
  const evaluateDropZone = (
    ...
    if (
      draggedComponent.dataMagicpathPath !== targetComponent.dataMagicpathPath
    ) {
      return {
        containerId: targetContainerId,
        isValid: false,
    ...
    if (
      ...
      return {
        ...
        isValid: false,
        reason:
          'Template instances can only be moved within template containers',
      }
    }

    if (
      draggedComponent?.type === 'item' &&
      targetComponent.type === 'collection-container'
    )
      return {
        containerId: targetContainerId,
        isValid: false,
  ...
  function isDescendant(activeItemId: string, dropTargetId: string): boolean {
    ...
    const recursiveSearch = (
      ...
      const childrenIds: string[] =
        nodeRegistry.containerToChildren[activeItemId]

      if (!childrenIds) return false

      if (childrenIds.includes(dropTargetId)) return true

      const childrenContainerIds = childrenIds.filter((id) =>
        IdGenerator.isContainerId(id)
      )

      return childrenContainerIds.some((id) =>
        recursiveSearch(id, dropTargetId)
      )
    }
