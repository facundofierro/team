...
# Build script for TeamHub MCP Runtime Docker image

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

IMAGE_NAME="teamhub-mcp-runtime"
IMAGE_TAG="${1:-latest}"
FULL_IMAGE_NAME="$IMAGE_NAME:$IMAGE_TAG"

echo "Building TeamHub MCP Runtime Docker image..."
...
echo "  docker run -d --name teamhub-mcp-org123 \\"
