...
export function MoveProvider({ children }: { children: React.ReactNode }) {
  ...
  const [isEditMode, setIsEditMode] = useState(
    (window as any).__editModeActive || false
  )
  ...
  const addMove = (moveData: Omit<AddMove, 'timestamp'>) => {
    const { itemId, sourceContainerId, destContainerId, newIndex, oldIndex } =
      moveData
  ...
  const moveNode = ({
    ...
    setNodeRegistry((prev) => {
      ...
      const idx = srcChildren.indexOf(itemId.toString())
      if (idx === -1) return prev

      srcChildren.splice(idx, 1)
      destChildren.splice(newIndex, 0, itemId.toString())
      ...
      const nextItemToParent =
        srcId === dstId
          ? prev.itemToParent
          : { ...prev.itemToParent, [itemId.toString()]: dstId.toString() }
