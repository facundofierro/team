...
echo -e "${BLUE}🔄 Docker-based Volume Migration Script: teamhub_ → agelum_${NC}"
echo "========================================================"

# Function to migrate a volume using Docker
migrate_volume_docker() {
    local OLD_VOLUME="$1"
    local NEW_VOLUME="$2"
    local DESCRIPTION="$3"
    ...
    # Copy data from old to new volume using Docker
...
# Function to backup old volume before migration using Docker
backup_volume_docker() {
    local VOLUME_NAME="$1"
...
backup_volume_docker "teamhub_postgres_data"
backup_volume_docker "teamhub_redis_data"

echo ""

# Migrate core infrastructure volumes
echo -e "${BLUE}🔧 Migrating core infrastructure volumes...${NC}"
migrate_volume_docker "teamhub_postgres_data" "agelum_postgres_data" "PostgreSQL data"
migrate_volume_docker "teamhub_redis_data" "agelum_redis_data" "Redis data"
