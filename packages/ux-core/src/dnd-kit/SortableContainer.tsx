...
export const SortableContainer = memo(
  forwardRef<HTMLElement, SortableContainerProps>(
    (
      { dndKitId, children, prevTag, containerType, ...props },
      forwardedRef
    ) => {
      const {
        registerContainer,
      ...
      const [sortingStrategy, setSortingStrategy] =
        useState<null | SORTING_STRAGETY>(null)
      ...
      const containerId = useMemo(() => {
        return generateIdForContainer(
          ...
          containerType as CONTANER_TYPE
        )
      ...
      useDndMonitor({
        onDragMove({ over, activatorEvent, delta }) {
          if (
            !over ||
            !isOutsideOfContainer ||
            !isValidDropTarget(over.id.toString())
          ) {
          ...
          for (let i = 0; i < childIds.length; i++) {
            ...
            if (!r) {
              ...
              if (
                (isVertical && pointerY < midpoint) ||
                (!isVertical && pointerX < midpoint)
              ) {
                newIndex = i
                break
              }
        ...
        onDragEnd({ active, over }) {
          ...
          if (!isOutsideOfContainer) {
            if (active.data.current?.sortable.containerId !== containerId)
              return
      ...
      useEffect(() => {
        processedChildren.forEach((child, index) => {
          ...
          const isTemplateInstance = IdGenerator.isTemplateId(
            child.id.toString()
          )
        ...
        return childIds.map((id, index) => {
          ...
          return (
            <SortableItem
              ...
              highlightTop={
                sortingStrategy === SORTING_STRAGETY.VERTICAL &&
          ...
          return createElement(
            ...
            <ParentIdContext.Provider value={containerId}>
              <SortableContext
                ...
                items={childIds}
                strategy={
                  sortingStrategyMappers[
                    sortingStrategy || SORTING_STRAGETY.HORIZONTAL
                  ]
                }
              >
                {content}
